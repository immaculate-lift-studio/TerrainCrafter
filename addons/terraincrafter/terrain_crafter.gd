...
@export var medium_lod_distance: float = 4096.0 
...
var loading_screen_visible: bool = true
...
func _ready() -> void:
	...
	chunk_creation_timer.wait_time = 0.03  # Adjust the interval for speed of chunk generation
...
func _process(_delta: float) -> void:
	_update_chunks()
	if loading_screen_visible and chunks.size() >= max_chunks:
...
func _generate_initial_chunks() -> void:
	...
	var end_x: int = start_x + 2 * render_distance + 1
	var end_z: int = start_z + 2 * render_distance + 1

	max_chunks = (2 * render_distance + 1) * (2 * render_distance + 1) 
...
func _update_chunks() -> void:
	...
	var removal_distance = render_distance + 3  # Set this to higher number if chunks delete constantly when you're not moving
